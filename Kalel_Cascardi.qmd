...
# Join types and stats
joined <- stats_df |> 
  left_join(types_df |> select(name, types), 
  by = "name", relationship = "many-to-many") |>
  left_join(abilities_df |> select(name, abilities), 
  by = "name", relationship = "many-to-many")
...
stats_by_ability <- joined |> 
  ...
  mutate(
    ...
    stat_label = factor(stat_label, levels = 
      c("attack", "defense", "hp", 
...
top_ability_per_type <- joined |> 
  ...
  ungroup() |>
  mutate(abilities = str_replace_all(abilities, "-", " "))  

plot2 <- ggplot(top_ability_per_type, aes(x = reorder(types, n), 
  y = n, fill = types)) +
  geom_col(show.legend = FALSE) +
  geom_text(aes(label = abilities), hjust = -0.1, size = 3.5, 
  fontface = "bold") +
  coord_flip() +
  labs(
...
#Addition- Samantha Mesa 
#Question: Do some Pokémon types rely heavily on a single stat, while others are more balanced across stats?
