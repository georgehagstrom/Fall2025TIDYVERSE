...
# Check that the API request was successful before continuing
# This helps catch issues like server errors, broken endpoints, or rate limits
res |>
  resp_check_status()

# If the request fails, the analysis stops here with a clear error message
# instead of silently returning incomplete or incorrect data
...
Additional Analysis by Hristiyana Yaneva

Since fat content can affect satiety and doesn't directly raise blood sugar, I wanted to examine which diabetes-friendly fruits are highest in fat, as this may help with meal planning and feeling fuller longer."

```{r}
#Fat content analysis of diabetes-friendly fruits
diabetes_friendly_fruits |>
  select(name, fat, sugar, carbohydrates) |>
  arrange(desc(fat)) |>
  slice_head(n = 10) |>
  ggplot(aes(x = fat, y = fct_reorder(name, fat))) +
  geom_col(fill = "darkgreen") +
  geom_text(aes(label = round(fat, 2), x = fat + 0.5), 
            size = 4, hjust = 0) +
  labs(
    title = "Fat Content in Diabetes-Friendly Fruits",
    subtitle = "Avocado stands out with healthy monounsaturated fats",
    x = "Fat (g per 100g)",
    y = "Fruit"
  ) +
  theme_minimal(base_size = 13)
...
Avocado is by far the highest in fat among diabetes-friendly fruits, but these are primarily heart-healthy monounsaturated fats. Most other low-sugar fruits contain minimal fat.
