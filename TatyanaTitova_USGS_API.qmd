...
## Extension by Denise Garbato: Magnitude vs Significance

**Objective: Explore how  physical earthquake size relates to the human impact.**

**Approach: explore relationship between magnitude and significance score**
...
```{r}
ggplot(eq_sig, aes(x = mag, y = sig)) +
geom_point(alpha = 0.4, color = "steelblue") +
geom_smooth(method = "lm", se = TRUE, color = "darkred") +
labs(
title = "Earthquake Magnitude vs Significance",
subtitle = "Higher magnitude earthquakes tend to have higher significance scores",
x = "Magnitude",
y = "USGS Significance Score"
) +
theme_light()
...
This scatter plot shows a strong positive relationship between earthquake magnitude
and the USGS significance score. As magnitude increases, earthquakes tend to have
substantially higher significance, indicating greater real-world impact.

However, for higher-magnitude earthquakes (approximately magnitude 6 and above),
there is noticeable variability in significance scores. Earthquakes of similar
magnitude can have very different significance values, suggesting that factors such
as population density, depth, and geographic location also play an important role
in determining an earthquake’s overall importance.

**Correlation between earthquake magnitude and significance**
```{r}
# Correlation between earthquake magnitude and significance
...
The Pearson correlation between earthquake magnitude and the USGS significance score
is 0.92, indicating a very strong positive linear relationship. This confirms the
visual pattern observed in the scatter plot, where higher-magnitude earthquakes
consistently correspond to higher significance values.

Despite this strong association, the relationship is not perfect. Earthquakes of
similar magnitude—especially at higher magnitudes—can have substantially different
significance scores. This suggests that while magnitude is a dominant driver of
significance, other factors such as population density, depth, and geographic
location also influence an earthquake’s overall impact.

**Conclusion**

This analysis shows a very strong positive association between earthquake magnitude
and the USGS significance score, with a Pearson correlation of 0.92. Higher-magnitude
earthquakes tend to have substantially higher significance, indicating that the
physical size of an earthquake is closely related to its real-world impact.

However, this relationship should not be interpreted as causal. The USGS significance
score incorporates multiple factors beyond magnitude, including population exposure,
felt reports, damage estimates, and media attention. As a result, earthquakes of
similar magnitude can have very different significance scores depending on their
depth, location, and surrounding infrastructure.

These findings highlight the distinction between physical earthquake size and
human-centered measures of impact. While magnitude is a dominant contributor to
significance, it does not fully determine an earthquake’s societal importance.
